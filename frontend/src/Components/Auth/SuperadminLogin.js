...
import { superadminlogin } from '../../actions/auth';
...
const SuperAdminLogin = () => {
  ...
  const handleSubmit = async e => {
    ...
    const success = await dispatch(superadminlogin(formData));
    if (success) {
      let route = `/dashboard/superadmin`;
      history.push(route)
    } else {
      setErrors(true);
    }
            ...
            <div className="card card-body">
              ...
              <form onSubmit={e => handleSubmit(e)}>
                ...
                <div className="form-group text-center mt-3 mb-2">
                  <button type="submit" className="btn btn-primary text-uppercase custom-label">{loading ? 'logging....' : 'Continue'}</button>
                </div>
                <div className="form-group">
